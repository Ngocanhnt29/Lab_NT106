...
namespace Lab4_ltmcb
{
    public partial class Bai6_Lab4 : Form
    {
        ...
        // Phương thức xử lý khi đăng nhập không thành công
        private async Task HandleUnsuccessfulLogin(HttpResponseMessage response)
        {
            var errorResponse = await response.Content.ReadAsStringAsync(); // Đọc nội dung phản hồi lỗi
            JObject errorJson = JObject.Parse(errorResponse); // Phân tích nội dung phản hồi lỗi thành đối tượng JSON
            string detail = errorJson["detail"].ToString(); // Lấy giá trị "detail" từ phản hồi lỗi
            richTextBox1.AppendText($"Lỗi: {detail}\n"); // Hiển thị thông báo lỗi trong richTextBox1
        }
        ...
        // Phương thức xử lý khi yêu cầu GET thông tin người dùng không thành công

        private async Task HandleUnsuccessfulUserInfoResponse(HttpResponseMessage userInfoResponse)
        {
            var errorResponse = await userInfoResponse.Content.ReadAsStringAsync(); // Đọc nội dung phản hồi lỗi
            JObject errorJson = JObject.Parse(errorResponse); // Phân tích nội dung phản hồi lỗi thành đối tượng JSON
            string detail = errorJson["detail"].ToString(); // Lấy giá trị "detail" từ phản hồi lỗi
            richTextBox1.AppendText($"Lỗi: {detail}\n"); // Hiển thị thông báo lỗi trong richTextBox1
        }
